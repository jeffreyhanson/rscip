...
// [[Rcpp::export]]
Rcpp::List rcpp_scip_solve(Rcpp::CharacterVector &modelsense, Rcpp::NumericVector &obj, Rcpp::NumericVector &lb,
                           Rcpp::NumericVector &ub, Rcpp::CharacterVector &vtype, Rcpp::NumericVector &rhs,
                           ...
                           bool use_initial_solution, Rcpp::IntegerVector &initial_index,
                           Rcpp::NumericVector &initial_solution, double gap = 0, double time_limit = 1e+20,
                           ...
                           bool verbose = true, std::size_t display_width = 143)
{
    ...
    for (std::size_t i = 0; i < n_consts; ++i)
    {
        ...
        //// every constraint
        k = 0;
        while (counter < n_nz && static_cast<std::size_t>(A_i[counter]) == i)
        {
            const std::size_t col = static_cast<std::size_t>(A_j[counter]);
            if (col >= n_vars)
            {
                Rcpp::stop("`A` has column indices outside variable bounds");
            }
            A_row_coefs[k] = A_x[counter];
            A_row_vars[k] = vars[col];
            ++k;
            ++counter;
        }
        ...
        SCIP_CALL(SCIPcreateConsBasicLinear(scip, &cons, std::to_string(i).c_str(), k, A_row_vars.data(),
                                            A_row_coefs.data(), const_lhs, const_rhs));
        SCIP_CALL(SCIPaddCons(scip, cons));
        constraints[i] = cons;
    }
    if (counter < n_nz)
    {
        Rcpp::stop("`A` has row indices outside constraint bounds");
    }
    ...
    if (use_initial_solution)
    {
        ...
        {
            SCIPsetSolVal(scip, initial_sol, vars[initial_index[i]], initial_solution[i]);
        }
    ...
    // Exports
    return Rcpp::List::create(Rcpp::Named("objval") = Rcpp::wrap(obj_val), Rcpp::Named("x") = Rcpp::wrap(x),
